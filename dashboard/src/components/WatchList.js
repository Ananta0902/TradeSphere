...
function WatchList() {
    return ( 
        <div className='watchlist-container'>
            ...
            placeholder="search eg: infy,bse,gold mcx" className='search' />
            <span className='counts'>{watchlist.length}/50 </span>
             </div>
            <ul className='list'>
                {watchlist.map((stock,index)=>{
               // <p>{stock.name}</p> //hard code default values
...
const WatchListItem=({stock})=>{
  //which stock is currently chosen-- show it or not
...
return(
  <li onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
    <div className='item'>
     ...
     {/* when stock hovered and uid unique for each stock */}
